...
class DataManager:
    ...
    def load_training_data(self):
        ...
        if self.basename == "task2_v1":
            path_to_shards = [os.path.join(self.data_dir, 'train', 'shard_1.tfrecord')]
        elif self.basename == "task1_v4":
            path_to_shards = [os.path.join(self.data_dir, 'train', 'shard_1.tfrecord')]
        else:
            print(len(path_to_shards))
            order = [6]
            path_to_shards = [path_to_shards[x] for x in order]
