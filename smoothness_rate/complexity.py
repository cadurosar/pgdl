...
def complexity(model, dataset):
	...
	def get_smoothness(inputs,labels):
		...
		noise = tf.random_normal(shape=tf.shape(inputs), mean=0.0, stddev=0.2, dtype=tf.float32) 
	...
	for i, (x, y) in enumerate(dataset.batch(500,drop_remainder=True)):
		...
		for ii in range(1,len(smoothnesses)):
			...
			smoothness_rate.append(smoothnesses[0])
		values.append(np.mean(smoothness_rate))
		if i == 1:  # only 10000 examples for efficiency
			break
